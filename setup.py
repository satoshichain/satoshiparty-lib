...
class move_old_db(Command):
    ...
    def run(self):
        ...
        old_data_dir = appdirs.user_config_dir(appauthor='Shellparty', appname='shellpartyd', roaming=True)
        old_database = os.path.join(old_data_dir, 'shellpartyd.9.db')
        old_database_testnet = os.path.join(old_data_dir, 'shellpartyd.9.testnet.db')

        new_data_dir = appdirs.user_data_dir(appauthor=config.SHP_NAME, appname=config.APP_NAME, roaming=True)
        ...
        # User have an old version of `shellpartyd`
...
setup_options = {
    ...
    'license': 'MIT',
    'description': 'Shellparty Protocol Reference Implementation',
    ...
    'download_url': 'https://github.com/ShellpartySHP/shellparty-lib/releases/tag/v' + CURRENT_VERSION,
    'provides': ['shellpartylib'],
