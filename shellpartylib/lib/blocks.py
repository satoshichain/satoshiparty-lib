...
Sieve blockchain for Shellparty transactions, and add them to the database.
"""
...
from SatoshiChain.core.script import CScriptInvalidError

from shellpartylib.lib import config
from shellpartylib.lib import exceptions
from shellpartylib.lib import util
from shellpartylib.lib import check
from shellpartylib.lib import script
from shellpartylib.lib import backend
from shellpartylib.lib import log
from shellpartylib.lib import database
from .messages import (send, order, shellpay, issuance, broadcast, bet, dividend, burn, cancel, rps, rpsresolve, publish, execute, destroy)

from .kickstart.blocks_parser import BlockchainParser, ChainstateParser
from .kickstart.utils import ib2h

from .exceptions import DecodeError, SCHOnlyError
...
TABLES = ['credits', 'debits', 'messages'] + \
         ...
         'bet_expirations', 'bets', 'broadcasts', 'shellpays', 'burns',
...
def parse_tx(db, tx):
    ...
    elif message_type_id == shellpay.ID:
        shellpay.parse(db, tx, message)
...
def initialise(db):
    ...
    cursor.execute('''CREATE INDEX IF NOT EXISTS
                   ...
                   ''')
    cursor.execute('''SELECT * FROM assets WHERE asset_name = ?''', ('SCH',))
    if not list(cursor):
        cursor.execute('''INSERT INTO assets VALUES (?,?,?)''', ('0', 'SCH', None))
        cursor.execute('''INSERT INTO assets VALUES (?,?,?)''', ('1', 'SHP', None))

    # Consolidated
    send.initialise(db)
    destroy.initialise(db)
    order.initialise(db)
    shellpay.initialise(db)
...
def get_tx_info1(tx_hex, block_index, block_parser=None):
    ...
    destination, shell_amount, data = None, None, b''
    ...
    for vout in ctx.vout:
        ...
        if not destination and not shell_amount and not data:
            address = get_address(vout.scriptPubKey)
            if address:
                destination = address
                shell_amount = vout.nValue
...
def list_tx(db, block_hash, block_index, block_time, tx_hash, tx_index, tx_hex=None):
    ...
    source, destination, shell_amount, fee, data = get_tx_info(tx_hex)
...
def kickstart(db, SatoshiChaind_dir):
    if SatoshiChaind_dir is None:
        if platform.system() == 'Darwin':
            SatoshiChaind_dir = os.path.expanduser('~/Library/Application Support/Shellcoin/')
        elif platform.system() == 'Windows':
            SatoshiChaind_dir = os.path.join(os.environ['APPDATA'], 'Shellcoin')
        else:
            SatoshiChaind_dir = os.path.expanduser('~/.SatoshiChain')
    if not os.path.isdir(SatoshiChaind_dir):
        raise Exception('Shellcoin Core data directory not found at {}. Use --SatoshiChaind-dir parameter.'.format(SatoshiChaind_dir))

    cursor = db.cursor()

    logger.warning('''Warning:
- Ensure that SatoshiChaind is stopped.
- You must reindex SatoshiChaind after the initialisation is complete (restart with `-reindex=1`)
- The initialisation may take a while.''')
    ...
    chain_parser = ChainstateParser(os.path.join(SatoshiChaind_dir, 'chainstate'))
    last_hash = chain_parser.get_last_block_hash()
    chain_parser.close()

    # Start block parser.
    block_parser = BlockchainParser(os.path.join(SatoshiChaind_dir, 'blocks'), os.path.join(SatoshiChaind_dir, 'blocks/index'))
    ...
    with db:
        ...
        while current_hash != None:
            ...
            for tx in block['transactions']:
                source, destination, shell_amount, fee, data = get_tx_info(tx['__data__'], block_parser=block_parser, block_index=block['block_index'])
                if source and (data or destination == config.UNSPENDABLE):
                    transactions.append((
                        tx['tx_hash'], block['block_index'], block['block_hash'], block['block_time'],
                        source, destination, shell_amount, fee, data
                    ))
            ...
            if len(transactions):
                ...
                for tx_chunk in tx_chunks:
                    sql = '''INSERT INTO transactions
                                (tx_index, tx_hash, block_index, block_hash, block_time, source, destination, shell_amount, fee, data) 
                             VALUES '''
                    bindings = ()
                    bindings_place = []
