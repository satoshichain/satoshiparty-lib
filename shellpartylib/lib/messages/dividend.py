...
from shellpartylib.lib import (config, exceptions, util)
...
def validate (db, source, quantity_per_unit, asset, dividend_asset, block_index):
    ...
    if asset == config.SCH:
        problems.append('cannot pay dividends to holders of {}'.format(config.SCH))
    if asset == config.SHP:
        if (not block_index >= 317500) or block_index >= 320000 or config.TESTNET:   # Protocol change.
            problems.append('cannot pay dividends to holders of {}'.format(config.SHP))
    ...
    if dividend_asset in (config.SCH, config.SHP):
    ...
    for holder in holders:
        ...
        if dividend_asset == config.SCH and dividend_quantity < config.DEFAULT_MULTISIG_DUST_SIZE: continue    # A bit hackish.
    ...
    fee = 0
    if not problems and dividend_asset != config.SCH:
        ...
        if fee:
            balances = list(cursor.execute('''SELECT * FROM balances WHERE (address = ? AND asset = ?)''', (source, config.SHP)))
            if not balances or balances[0]['quantity'] < fee:
                problems.append('insufficient funds ({})'.format(config.SHP))

    if not problems and dividend_asset == config.SHP:
...
def parse (db, tx, message):
    ...
    if dividend_asset == config.SCH:
        status = 'invalid: cannot pay {} dividends within protocol'.format(config.SCH)
    ...
    if status == 'valid':
        ...
        if tx['block_index'] >= 330000 or config.TESTNET: # Protocol change.
            util.debit(db, tx['source'], config.SHP, fee, action='dividend fee', event=tx['tx_hash'])
