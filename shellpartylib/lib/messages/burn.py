...
"""Burn {} to earn {} during a special period of time.""".format(config.SCH, config.SHP)
...
def compose (db, source, quantity, overburn=False):
    ...
    # Check that a maximum of 1 SCH total is burned per address.
    ...
    if quantity > (1 * config.UNIT - already_burned) and not overburn:
        raise exceptions.ComposeError('1 {} may be burned per address'.format(config.SCH))
