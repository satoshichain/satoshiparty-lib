...
def cancel_bet (db, bet, status, block_index):
    ...
    util.credit(db, bet['source'], config.SHP, bet['wager_remaining'], action='recredit wager remaining', event=bet['tx_hash'])
...
def parse (db, tx, message):
    ...
    if status == 'open':
        ...
        bet_parse_cursor.execute('''SELECT * FROM balances \
                                    WHERE (address = ? AND asset = ?)''', (tx['source'], config.SHP))
        ...
        if tx0_inverse_odds > tx1_odds:
            ...
            if tx1['block_index'] >= 292000 or config.TESTNET:  # Protocol change
                if tx1_wager_remaining <= 0 or tx1_counterwager_remaining <= 0:
                    # Fill order, and recredit give_remaining.
                    tx1_status = 'filled'
                    util.credit(db, tx1['source'], config.SHP, tx1_wager_remaining, event=tx1['tx_hash'], action='filled')
            # tx1
            bindings = {
