...
# Filled orders may not be re‐opened, so only orders not involving SCH (and so
...
def exact_penalty (db, address, block_index, order_match_id):
    # Penalize addresses that don’t make SCH payments. If an address lets an
    # order match expire, expire sell SCH orders from that address.
    cursor = db.cursor()

    # Orders.
    bad_orders = list(cursor.execute('''SELECT * FROM orders \
                                        WHERE (source = ? AND give_asset = ? AND status = ?)''',
                                     (address, config.SCH, 'open')))
    ...
    if not (block_index >= 314250 or config.TESTNET):   # Protocol change.
        ...
        bad_order_matches = list(cursor.execute('''SELECT * FROM order_matches \
                                         ...
                                         (address, config.SCH, address, config.SCH, 'pending')))
...
def validate (db, source, give_asset, give_quantity, get_asset, get_quantity, expiration, fee_required, block_index):
    ...
    if give_asset == config.SCH and get_asset == config.SCH:
        problems.append('cannot trade {} for itself'.format(config.SCH))
    ...
    if give_asset not in (config.SCH, config.SHP) and not cursor.fetchall():
    ...
    if get_asset not in (config.SCH, config.SHP) and not cursor.fetchall():
    ...
    for tx0 in order_matches:
        ...
        if tx0_price > tx1_inverse_price:
            ...
            if block_index >= 313900 or config.TESTNET: # Protocol change.
                ...
                if (forward_asset == config.SCH and forward_quantity <= min_shell_quantity) or (backward_asset == config.SCH and backward_quantity <= min_shell_quantity):
...
def expire (db, block_index):
    ...
    # Expire orders and give refunds for the quantity give_remaining (if non-zero; if not SCH).
    cursor.execute('''SELECT * FROM orders \
    ...
    # Expire order_matches for SCH with no SCH.
    cursor.execute('''SELECT * FROM order_matches \
