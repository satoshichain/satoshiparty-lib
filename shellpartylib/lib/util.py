...
from shellpartylib.lib.exceptions import DecodeError
...
def api(method, params):
    ...
    if response == None:
        raise RPCError('Cannot communicate with {} server.'.format(config.SHP_NAME))
...
def generate_asset_id(asset_name, block_index):
    ...
    if asset_name == config.SCH: return 0
    elif asset_name == config.SHP: return 1
...
def generate_asset_name (asset_id, block_index):
    ...
    if asset_id == 0: return config.SCH
    elif asset_id == 1: return config.SHP
...
def debit (db, address, asset, quantity, action=None, event=None):
    ...
    if asset == config.SCH:
        raise DebitError('Cannot debit SatoshiChains.')

    debit_cursor = db.cursor()

    # Contracts can only hold SHP balances.
    if enabled('contracts_only_shp_balances'): # Protocol change.
        if len(address) == 40:
            assert asset == config.SHP

    if asset == config.SCH:
        raise exceptions.BalanceError('Cannot debit SatoshiChains from a {} address!'.format(config.SHP_NAME))
...
def credit (db, address, asset, quantity, action=None, event=None):
    ...
    if asset == config.SCH:
        raise CreditError('Cannot debit SatoshiChains.')
...
def holders(db, asset):
    ...
    # Bets and RPS (and bet/rps matches) only escrow SHP.
    if asset == config.SHP:
        ...
        # SHP escrowed for not finished executions
...
def shp_created (db):
    """Return number of SHP created thus far."""
    cursor = db.cursor()
...
def shp_destroyed (db):
    """Return number of SHP destroyed thus far."""
    cursor = db.cursor()
    # Destructions
    cursor.execute('''SELECT SUM(quantity) AS total FROM destructions \
                      WHERE (status = ? AND asset = ?)''', ('valid', config.SHP))
...
def shp_supply (db):
    """Return the SHP supply."""
    return shp_created(db) - shp_destroyed(db)

def creations (db):
    """Return creations."""
    cursor = db.cursor()
    creations = {config.SHP: shp_created(db)}
...
def destructions (db):
    """Return destructions."""
    cursor = db.cursor()
    destructions = {config.SHP: shp_destroyed(db)}
    cursor.execute('''SELECT asset, SUM(quantity) AS destroyed FROM destructions \
                      WHERE (status = ? AND asset != ?) GROUP BY asset''', ('valid', config.SHP))
...
def held (db): #TODO: Rename ?
    sql = '''SELECT asset, SUM(total) AS total FROM (
                ...
                UNION ALL
                SELECT 'SHP' AS asset, SUM(wager_remaining) AS total FROM bets WHERE status = 'open'
                UNION ALL
                SELECT 'SHP' AS asset, SUM(forward_quantity) AS total FROM bet_matches WHERE status = 'pending'
                UNION ALL
                SELECT 'SHP' AS asset, SUM(backward_quantity) AS total FROM bet_matches WHERE status = 'pending'
                UNION ALL
                SELECT 'SHP' AS asset, SUM(wager) AS total FROM rps WHERE status = 'open'
                UNION ALL
                SELECT 'SHP' AS asset, SUM(wager * 2) AS total FROM rps_matches WHERE status IN ('pending', 'pending and resolved', 'resolved and pending')
                UNION ALL
                SELECT 'SHP' AS asset, SUM(gas_cost) AS total FROM executions WHERE status IN ('valid', 'out of gas')
                UNION ALL
                SELECT 'SHP' AS asset, SUM(gas_remained) AS total FROM executions WHERE status  = 'out of gas'
            ) GROUP BY asset;'''
