...
from SatoshiChain.core.key import CPubKey
...
def get_asm(scriptpubkey):
    ...
    try:
        ...
        for op in scriptpubkey:
            if type(op) == SatoshiChainlib.core.script.CScriptOp:
                # TODO: `op = element`
                asm.append(str(op))
            else:
                # TODO: `data = element` (?)
                asm.append(op)
    except SatoshiChainlib.core.script.CScriptTruncatedPushDataError:
...
# TODO: Use `python-SatoshiChaintools` instead. (Get rid of `pycoin` dependency.)
