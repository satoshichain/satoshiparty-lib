...
import server
from shellpartylib.lib import (config, api, util, exceptions, blocks, check, backend, database, transaction, script)
from shellpartylib.lib.messages import (send, order, shellpay, issuance, broadcast, bet, dividend, burn, cancel, rps, rpsresolve)
...
def insert_raw_transaction(raw_transaction, db, rawtransactions_db):
    ...
    source, destination, shell_amount, fee, data = blocks.get_tx_info2(raw_transaction)
    transaction = (tx_index, tx_hash, block_index, block_hash, block_time, source, destination, shell_amount, fee, data, True)
    ...
    if pytest.config.option.savescenarios:
        ...
        with open(CURR_DIR + '/fixtures/unspent_outputs.json', 'r') as listunspent_test_file:
                ...
                for output in wallet_unspent:
                    txid = binascii.hexlify(SatoshiChainlib.core.lx(output['txid'])).decode()
...
def save_rawtransaction(db, tx_hash, tx_hex):
    ...
    cursor = db.cursor()
    try:
        txid = binascii.hexlify(SatoshiChainlib.core.lx(tx_hash)).decode()
