...
from shellpartylib.lib import config, util, backend, transaction
...
def init_mock_functions(monkeypatch, rawtransactions_db):
    ...
    def get_cached_raw_transaction(tx_hash, verbose=False):
        return util_test.getrawtransaction(rawtransactions_db, SatoshiChainlib.core.lx(tx_hash))

    util.CURRENT_BLOCK_INDEX = DEFAULT_PARAMS['default_block'] - 1

    monkeypatch.setattr('shellpartylib.lib.backend.get_unspent_txouts', get_unspent_txouts)
    monkeypatch.setattr('shellpartylib.lib.log.isodt', isodt)
    monkeypatch.setattr('shellpartylib.lib.log.curr_time', curr_time)
    monkeypatch.setattr('shellpartylib.lib.util.date_passed', date_passed)
    monkeypatch.setattr('shellpartylib.lib.api.init_api_access_log', init_api_access_log)
    if hasattr(config, 'PREFIX'):
        monkeypatch.setattr('shellpartylib.lib.config.PREFIX', b'TESTXXXX')
    monkeypatch.setattr('shellpartylib.lib.backend.getrawtransaction', get_cached_raw_transaction)
    monkeypatch.setattr('shellpartylib.lib.backend.pubkeyhash_to_pubkey', pubkeyhash_to_pubkey)
    monkeypatch.setattr('shellpartylib.lib.backend.multisig_pubkeyhashes_to_pubkeys', multisig_pubkeyhashes_to_pubkeys)
